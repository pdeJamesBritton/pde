 ...
 * @brief this is copied past from the links below.  Full credit to be giving to previous author
 */
...
int main() {
 ...
 //   std::cout<< x << std::endl;
    ...
    auto model = net.model();
//    model.eval();
//    torch.nograd();
//    auto y_pred = model(X).reshape(x.size(1), y.size(1));
//    std::cout << "max element of y_pred: " << y_pred.max() << std::endl;    
